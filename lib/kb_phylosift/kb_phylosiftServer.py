...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'kb_phylosift'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from kb_phylosift.kb_phylosiftImpl import kb_phylosift
impl_kb_phylosift = kb_phylosift(config)
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'kb_phylosift'
        ...
        self.rpc_service.add(impl_kb_phylosift.run_phylosift,
                             name='kb_phylosift.run_phylosift',
                             types=[dict])
        self.method_authentication['kb_phylosift.run_phylosift'] = 'required'
        self.rpc_service.add(impl_kb_phylosift.status,
                             name='kb_phylosift.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "kb_phylosift but no authentication header was passed"
                            raise err
